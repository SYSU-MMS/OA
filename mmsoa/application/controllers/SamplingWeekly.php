...
Class SamplingWeekly extends CI_Controller {
	public function __construct() {
		parent::__construct();
		$this->load->model('Moa_user_model');
		$this->load->model('Moa_worker_model');
		$this->load->model('Moa_sampling_weekly_model');
	...
	public function index() {
		if (isset($_SESSION['user_id'])) {
			...
			$this->load->view('view_sampling_weekly_list');
		} else {
			// 未登录的用户请先登录
	...
	public function newTable() {
		if (isset($_SESSION['user_id'])) {
			...
			if (isset($group_a) && isset($group_b)) {
				...
				$res = $this->Moa_sampling_weekly_model->add_new_table($table_list);
				if ($res === false) {
	...
	public function getTableList() {
		if (isset($_SESSION['user_id'])) {
			...
			do {
				$tmp_stamp = $this->Moa_sampling_weekly_model->get_by_date($today, 1, 1, 0);
	...
	public function getTable() {
		if (isset($_SESSION['user_id'])) {
			...
			if (!isset($_POST['timestring'])) {
				...
				$sample_object_list = $this->Moa_sampling_weekly_model->get_table($date);
	...
	public function showTable($timestring) {
		if (isset($_SESSION['user_id'])) {
			...
			$this->load->view('view_sampling_weekly_table', array('data' => $timestring, 'title' => $title));
	...
	public function deleteTable() {
		if (isset($_SESSION['user_id'])) {
			...
			if (!isset($_POST['timestamp'])) {
				...
				$old_table = $this->Moa_sampling_weekly_model->get_table($_POST['timestamp']);
				$len = count($old_table);
				for ($i = 0; $i < $len; ++$i) {
				...
				$ret = $this->Moa_sampling_weekly_model->delete_table($_POST['timestamp']);
				if ($ret != false) {
	...
	public function upDateRecord() {
		if (isset($_SESSION['user_id'])) {
			...
			$old_record = $this->Moa_sampling_weekly_model->get($_POST['sid']);
			...
			if ($_POST['state'] != 0) {
				...
				if ($_POST['state'] != 1 && $old_record[0]->state == 1) {
					$this->Moa_worker_model->update_weekly_perfect($wid, -1);
                    $this->Moa_user_model->update_weekly_perfect($old_record[0]->target_uid, -1);
			...
			if ($old_record[0]->operator_uid == NULL) {
				$this->Moa_worker_model->update_weekly_check($wid, 1);
                $this->Moa_user_model->update_weekly_check($old_record[0]->target_uid, 1);
			}

			$record['problem'] = $_POST['problem'];
			$record['operator_uid'] = $_SESSION['user_id'];

			$ret = $this->Moa_sampling_weekly_model->update_a_record($record, $_POST['sid']);
            if ($ret == false) {
	...
	public function monthSamplingRank() {
		if (isset($_SESSION['user_id'])) {
			$rankList = $this->Moa_sampling_weekly_model->get_rank_month();
			$nameList = array();
			$perList = array();
			$perfectList = array();
			$checkList = array();
			for ($i = 0; $i < count($rankList); $i++) {
				...
				$perfectList[$i] = $rankList[$i]->wperfect;
				$checkList[$i] = $rankList[$i]->wchecks;
				if ($rankList[$i]->wchecks == 0) {
					$perList[$i] = '0.00%';
				} else {
					$perList[$i] = sprintf('%.2f', ($rankList[$i]->wperfect / $rankList[$i]->wchecks * 100.0)) . '%';
				}
			}
			...
			$this->load->view('view_sampling_weekly_statistics', $viewparas);
		} else {
			// 未登录的用户请先登录
			PublicMethod::requireLogin();
			return;
	...
	public function allSamplingRank() {
		if (isset($_SESSION['user_id'])) {
			$rankList = $this->Moa_sampling_weekly_model->get_rank_all();
			$nameList = array();
			$perList = array();
			$perfectList = array();
			$checkList = array();
			for ($i = 0; $i < count($rankList); $i++) {
				$userObj = $this->Moa_user_model->get($rankList[$i]->uid);
				$nameList[$i] = $userObj->name;
				$perfectList[$i] = $rankList[$i]->totalwPerfect;
				$checkList[$i] = $rankList[$i]->totalwCheck;
				if ($rankList[$i]->totalwCheck == 0) {
					$perList[$i] = '0.00%';
				} else {
					$perList[$i] = sprintf('%.2f', ($rankList[$i]->totalwPerfect / $rankList[$i]->totalwCheck * 100.0)) . '%';
				}
			}
			$viewparas['perfectlist'] = $perfectList;
			$viewparas['checklist'] = $checkList;
